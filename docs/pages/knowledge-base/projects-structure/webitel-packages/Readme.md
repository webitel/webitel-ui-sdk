...
### Лінкуванння
Лінкуємо лібу чи певний пакета (або все разом) до апплікейшена, на якому воно потрібно.
Кроки відтворення:
1. незалежно від того який пакет потрібен - в корені `@webitel/ui-sdk` виконуємо `npm run build`
2. там же виконуємо `npm link`
3. в необхідному аплікейшині робимо [`npm link назва-пакету`](https://docs.npmjs.com/cli/v9/commands/npm-link).
...
> 3. якщо новий пакет чи нова машина - `npm link` треба зробити саме в корені цього пакету
