...
import type {
	...
	ResetListCoreOptions,
	useTableStoreConfig,
} from '../types/tableStore.types';

/**
 * List-core: the seam presets and permissions already compose with.
 * Two adapters justify it — registry (`createTableStore`) and the package
 * extensions that spread this body. Nest identity (`parentId`) and `$reset`
 * live here so adapters do not fork them.
 */
export const createListCore = <Entity extends Identifiable>(
	namespace: string,
	...
	const parentId = ref<string | number | undefined>();
	/**
    ...
    */
	 ...
	 *
	 * Headers stay by default — they are the user's column choice, restored once
	 * per app lifetime — and so does `isStoreSetUp`. Pass `{ headers: true }` when
	 * an adapter (permissions) must wipe column state too.
	 */
	const $reset = ({
		headers: wipeHeaders = false,
	}: ResetListCoreOptions = {}) => {
		dataList.value = [];
		selected.value = [];
		error.value = null;
	...
	return {
		parentId,
		isStoreSetUp, // pinia devtools
		isNested, // pinia devtools
...
/** @deprecated use createListCore */
export const tableStoreBody = createListCore;
