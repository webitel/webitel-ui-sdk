 ...
 */
export const searchTypesQuerySortItemDefault = "*";
export const searchTypesQuerySortItemRegExp = new RegExp('^[+|-|!]?\\w+$');
export const searchTypesQueryFieldsItemDefault = "*";

export const searchTypesQueryParams = zod.object({
  ...
  "sort": zod.array(zod.string().regex(searchTypesQuerySortItemRegExp)).optional().describe('Sort result dataset of records by fields.\n```\nsort ::= *( ORDER name )\n\nORDER  = ASC / DESC\nDESC   = \"-\" / \"!\"\nASC    = [ \"+\" ]   ; Default\n```\n\nFields available\n\n- `id`(seq)\n- `domain`{name}\n- `created_at`\n- `created_by`{name}\n- `updated_at`\n- `updated_by`{name}\n\nUse ?fields=`field.sort()` option to sort Edge fields.'),
...
export const searchTypesResponseDataItemFieldsItemKindDefault = "none";

export const searchTypesResponse = zod.object({
  ...
  "kind": zod.enum(['none', 'list', 'bool', 'int', 'int32', 'int64', 'uint', 'uint32', 'uint64', 'float', 'float32', 'float64', 'binary', 'lookup', 'string', 'richtext', 'datetime', 'duration']).default(searchTypesResponseDataItemFieldsItemKindDefault).describe('Kind of primitive data types.\n\n - none: option allow_alias = true;\n - list: [array]\n - int: int32\n - uint: uint32\n - float: float32\n - datetime: date &| time'),
  "lookup": zod.object({
...
export const locateResponse = zod.object({
  ...
  "kind": zod.enum(['none', 'list', 'bool', 'int', 'int32', 'int64', 'uint', 'uint32', 'uint64', 'float', 'float32', 'float64', 'binary', 'lookup', 'string', 'richtext', 'datetime', 'duration']).default(locateResponseFieldsItemKindDefault).describe('Kind of primitive data types.\n\n - none: option allow_alias = true;\n - list: [array]\n - int: int32\n - uint: uint32\n - float: float32\n - datetime: date &| time'),
  "lookup": zod.object({
  "display": zod.string().optional().describe('[Readonly]. Display dataset field.\n(lookup).{`name`} value relation.'),
  "name": zod.string().optional().describe('[Readonly]. Dataset title.'),
  "path": zod.string().optional().describe('[Required]. Reference dataset relative path\ne.g.: \"contacts\", \"dictionaries/cities\".\n(lookup).{`type`} value relation.'),
