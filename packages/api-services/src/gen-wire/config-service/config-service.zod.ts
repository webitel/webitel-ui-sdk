/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Webitel API
 * OpenAPI spec version: 24.04.0
 */
import * as zod from 'zod';

export const ConfigServiceReadSystemObjectsQueryParams = zod.object({
	include_existing: zod.boolean().optional(),
	page: zod.int().optional(),
	size: zod.int().optional(),
	q: zod.string().optional(),
	sort: zod.string().optional(),
	fields: zod.array(zod.string()).optional(),
});

export const ConfigServiceReadSystemObjectsResponse = zod.object({
	items: zod
		.array(
			zod.object({
				id: zod.int().optional(),
				name: zod.string().optional(),
			}),
		)
		.optional(),
});

export const ConfigServiceSearchConfigQueryParams = zod.object({
	page: zod.int().optional(),
	size: zod.int().optional(),
	q: zod.string().optional(),
	sort: zod.string().optional(),
	fields: zod.array(zod.string()).optional(),
	object: zod
		.array(
			zod.enum([
				'cc_queue',
				'schema',
				'users',
				'devices',
				'calendars',
				'cc_list',
				'cc_team',
				'cc_agent',
				'cc_resource',
				'cc_resource_group',
				'chat_bots',
				'cases',
				'contacts',
				'cc_list_number',
				'case_comments',
				'record_file',
				'sso',
			]),
		)
		.optional()
		.describe('NOT USED'),
});

export const ConfigServiceSearchConfigResponse = zod.object({
	items: zod
		.array(
			zod.object({
				days_to_store: zod.int().optional(),
				description: zod.string().optional(),
				enabled: zod.boolean().optional(),
				id: zod.int().optional(),
				logs_count: zod.string().optional(),
				logs_size: zod.string().optional(),
				object: zod
					.object({
						id: zod.int().optional(),
						name: zod.string().optional(),
					})
					.optional(),
				period: zod.int().optional(),
				storage: zod
					.object({
						id: zod.int().optional(),
						name: zod.string().optional(),
					})
					.optional(),
			}),
		)
		.optional(),
	next: zod.boolean().optional(),
	page: zod.int().optional(),
});

export const ConfigServiceCreateConfigBody = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	object: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServiceCreateConfigResponse = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	id: zod.int().optional(),
	logs_count: zod.string().optional(),
	logs_size: zod.string().optional(),
	object: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServiceDeleteConfigParams = zod.object({
	config_id: zod.int(),
});

export const ConfigServiceDeleteConfigResponse = zod.looseObject({});

export const ConfigServiceReadConfigParams = zod.object({
	config_id: zod.int().describe('int32 domainId = 8;'),
});

export const ConfigServiceReadConfigResponse = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	id: zod.int().optional(),
	logs_count: zod.string().optional(),
	logs_size: zod.string().optional(),
	object: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServicePatchConfigParams = zod.object({
	config_id: zod.int(),
});

export const ConfigServicePatchConfigBody = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	fields: zod.array(zod.string()).optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServicePatchConfigResponse = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	id: zod.int().optional(),
	logs_count: zod.string().optional(),
	logs_size: zod.string().optional(),
	object: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServiceUpdateConfigParams = zod.object({
	config_id: zod.int(),
});

export const ConfigServiceUpdateConfigBody = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});

export const ConfigServiceUpdateConfigResponse = zod.object({
	days_to_store: zod.int().optional(),
	description: zod.string().optional(),
	enabled: zod.boolean().optional(),
	id: zod.int().optional(),
	logs_count: zod.string().optional(),
	logs_size: zod.string().optional(),
	object: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
	period: zod.int().optional(),
	storage: zod
		.object({
			id: zod.int().optional(),
			name: zod.string().optional(),
		})
		.optional(),
});
