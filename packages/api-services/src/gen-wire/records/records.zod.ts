...
export const _DeleteResponse = zod
	.object({
		...
		type: zod
					...
					.array(
						zod
							.object({
								...
								binary: zod
									.object({
										max_bytes: zod.int().optional(),
										violation: zod
											.record(zod.string(), zod.string())
											.optional(),
									})
									.optional(),
								bool: zod.looseObject({}).optional(),
								datetime: zod
									.object({
										epoch: zod
											.number()
								...
								kind: zod
									...
									.describe('Required. The field type.'),
								lookup: zod
									.object({
										display: zod
											.string()
			...
			.optional()
			.describe('Type of the dataset record.'),
	})
 ...
 */
export const SearchRecordsParams = zod.object({
	repo: zod.string().describe('[`types.repo`]'),
});

export const searchRecordsQuerySortItemDefault = `*`;
export const searchRecordsQuerySortItemRegExp = /^[+|-|!]?\w+$/;
export const searchRecordsQueryFieldsItemDefault = `*`;
...
export const SearchRecordsQueryParams = zod.object({
	...
	sort: zod
		.array(
			zod
				.string()
				.regex(searchRecordsQuerySortItemRegExp)
				.default(searchRecordsQuerySortItemDefault),
		)
		.optional()
		.describe(
	...
	fields: zod
		.array(zod.string().default(searchRecordsQueryFieldsItemDefault))
		.optional()
		.describe(
...
export const searchRecordsResponseTypeFieldsItemKindDefault = `none`;

export const SearchRecordsResponse = zod
	.object({
		data: zod
			.array(zod.looseObject({}))
			.optional()
						...
						zod
							.object({
								...
								kind: zod
									...
									.default(searchRecordsResponseTypeFieldsItemKindDefault)
									.describe('Required. The field type.'),
								lookup: zod
									.object({
										display: zod
...
export const CreateBody = zod.looseObject({});

export const CreateResponse = zod.looseObject({});

/**
 * (Record) {
 * @summary Locate dataset record
 */
export const LocateRecordsParams = zod.object({
	repo: zod.string().describe('[`types.repo`]'),
	id: zod.string().describe('[`record.id`]'),
});

export const locateRecordsQueryFieldsItemDefault = `*`;

export const LocateRecordsQueryParams = zod.object({
	fields: zod
		.array(zod.string().default(locateRecordsQueryFieldsItemDefault))
		.optional()
		.describe(
			'Fields [Q]uery to build result dataset record.\n```\nfields ::= field [ *( "," field ) ]\nfield  ::= name [ *( func ) ] [ inner ]\ninner  ::= "{" fields "}"\nfuncs  ::= *( func )\nfunc   ::= "." name "(" [ args ] ")"\nname   ::= ALPHA / DIGIT / USCORE\n\nALPHA    = %x41-5A / %x61-7A  ; "A"-"Z" / "a"-"z"\nDIGIT    = %x30-39            ; "0"-"9"\nUSCORE   = %x5F ; underscore  ; "_"\n```',
		),
...
export const LocateRecordsResponse = zod.looseObject({});
...
export const UpdateBody = zod.looseObject({});

export const UpdateResponse = zod.looseObject({});
...
export const Update2Body = zod.looseObject({});

export const Update2Response = zod.looseObject({});
