...
export const SearchDataQueryParams = zod.object({
	...
	sort: zod
		.array(
			zod
				.string()
				.regex(searchDataQuerySortItemRegExp)
				.default(searchDataQuerySortItemDefault),
		)
		.optional()
		.describe(
	...
	fields: zod
		.array(zod.string().default(searchDataQueryFieldsItemDefault))
		.optional()
		.describe(
...
export const CreateDataBody = zod.looseObject({});
...
export const BatchCreateDataResponse = zod.object({
	...
	failed: zod.int().optional().describe('Number of failed rows.'),
	imported: zod
		.int()
		.optional()
		.describe('Number of successfully imported rows.'),
	total: zod.int().optional().describe('Total number of input rows.'),
});
...
export const ImportCSVBody = zod.object({
	...
	fields: zod.record(zod.string(), zod.int()).optional(),
	line_begin: zod.int().optional(),
	line_end: zod.int().optional(),
	on_data_error: zod
		.enum([
			'continue',
			'break',
...
export const LocateDataQueryParams = zod.object({
	fields: zod
		.array(zod.string().default(locateDataQueryFieldsItemDefault))
		.optional()
		.describe(
			'Fields [Q]uery to build result dataset record.\n```\nfields ::= field [ *( "," field ) ]\nfield  ::= name [ *( func ) ] [ inner ]\ninner  ::= "{" fields "}"\nfuncs  ::= *( func )\nfunc   ::= "." name "(" [ args ] ")"\nname   ::= ALPHA / DIGIT / USCORE\n\nALPHA    = %x41-5A / %x61-7A  ; "A"-"Z" / "a"-"z"\nDIGIT    = %x30-39            ; "0"-"9"\nUSCORE   = %x5F ; underscore  ; "_"\n```',
		),
...
export const LocateDataResponse = zod.looseObject({});
...
export const UpdateDataBody = zod.looseObject({});

export const UpdateDataResponse = zod.looseObject({});
...
export const UpdateData2Body = zod.looseObject({});

export const UpdateData2Response = zod.looseObject({});
...
export const DeleteTypeResponse = zod
	.object({
		...
		next: zod.boolean().optional(),
		page: zod.int().optional().describe('Page number of results.'),
	})
...
export const SearchTypeQueryParams = zod.object({
	...
	sort: zod
		.array(
			zod
				.string()
				.regex(searchTypeQuerySortItemRegExp)
				.default(searchTypeQuerySortItemDefault),
		)
		.optional()
		.describe(
			'Sort result dataset of records by fields.\n```\nsort ::= *( ORDER name )\n\nORDER  = ASC / DESC\nDESC   = "-" / "!"\nASC    = [ "+" ]   ; Default\n```\n\nFields available\n\n- `id`(seq)\n- `domain`{name}\n- `created_at`\n- `created_by`{name}\n- `updated_at`\n- `updated_by`{name}\n\nUse ?fields=`field.sort()` option to sort Edge fields.',
	...
	fields: zod
		.array(zod.string().default(searchTypeQueryFieldsItemDefault))
		.optional()
		.describe(
			'Fields [Q]uery to build result dataset record.\n```\nfields ::= field [ *( "," field ) ]\nfield  ::= name [ *( func ) ] [ inner ]\ninner  ::= "{" fields "}"\nfuncs  ::= *( func )\nfunc   ::= "." name "(" [ args ] ")"\nname   ::= ALPHA / DIGIT / USCORE\n\nALPHA    = %x41-5A / %x61-7A  ; "A"-"Z" / "a"-"z"\nDIGIT    = %x30-39            ; "0"-"9"\nUSCORE   = %x5F ; underscore  ; "_"\n```',
		),
