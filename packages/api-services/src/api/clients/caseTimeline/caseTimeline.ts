import { getCaseTimeline } from '@webitel/api-services/gen';
import {
	applyTransform,
	merge,
	notify,
...
const getList = async (params) => {
	...
	try {
		const response = await getCaseTimeline().getTimeline(parentId, {
			page: page || 1,
			size: size || 10,
			dateFrom,
			dateTo,
...
const getCounters = async (params) => {
	...
	try {
		const response = await getCaseTimeline().getTimelineCounter(parentId);
