...
import { AdminSectionsValues, AuditorSectionsValues, CrmSectionsValues, mapGlobalActionToCrudAction, mapScopeClassAccessTokenToCrudAction, mapScopeClassToWtObjects, mapUiSectionToWtObject, mapWtObjectToUiSection, SupervisorSectionsValues } from "../mappings/mappings";
/**
 * @internal
 * @description
 * backend -> frontend
...
export const makeScopeAccessMap = (rawScope) => {
    ...
    rawScope.forEach(({ class: scopeClass, access: scopeAccess }) => {
        ...
        if (!scopeClassObjects) {
            wtlog.error({ module: 'modules/userinfo' })('Unknown scope class to convert to WtObject:', scopeClass);
        }
        else {
    ...
    Object.values(rawVisibility).forEach((appSectionsVisibility) => {
        Object.entries(appSectionsVisibility).forEach(([section, visibility]) => {
            if (section.startsWith('_'))
                return map; // skip private fields
