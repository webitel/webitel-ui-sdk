		...
		validationText() {
			...
			if (this.isValidation && this.invalid) {
				if (this.v.required?.$invalid)
					...
					validationText = this.$t('validation.minValue', {
						min: this.v.minValue.$params.min,
					});
				else if (this.v.maxValue?.$invalid)
					validationText = this.$t('validation.maxValue', {
						max: this.v.maxValue.$params.max,
					});
					...
					validationText = this.$t('validation.minLength', {
						min: this.v.minLength.$params.min,
					});
				else if (this.v.maxLength?.$invalid)
					validationText = this.$t('validation.maxLength', {
						max: this.v.maxLength.$params.max,
					});
