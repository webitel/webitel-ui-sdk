<template>
  <div :class="{
    ...
    <wt-label v-if="hasLabel" :disabled="disabled" :invalid="invalid" class="wt-tags-input__label" v-bind="labelProps">
    ...
    <vue-multiselect ref="vue-multiselect" :close-on-select="false" :disabled="disabled"
      :internal-search="!searchMethod" :label="selectOptionLabel" :loading="false" :model-value="selectValue"
      :multiple="multiple" :options="selectOptions" :placeholder="placeholder || label" :taggable="taggable"
      :track-by="trackBy" class="wt-tags-input__select" v-bind="$attrs" v-on="listeners">
      ...
      <template #tag="{ option, remove }">
        <wt-chip class="multiselect__custom-tag">
          ...
          <wt-icon-btn color="on-primary" icon="close--filled" size="sm" @click="remove(option)" />
        </wt-chip>
      </template>
      ...
      <template #caret="{ toggle }">
        ...
        <wt-icon-btn :disabled="disabled" class="multiselect__select" color="active" icon="arrow-down"
