...
const iconsRepository = new Map<IconRepositoryEntry['iconName'], IconRepositoryEntry['svg']>();
...
/* export */ function addIconToRepository({ iconName, svg }: IconRepositoryEntry) {
  iconsRepository.set(iconName, dompurify.sanitize(svg));
};

export function fillIconsRepository({ icons }: { icons: IconRepositoryEntry[] }) {
  icons.forEach((entry) => {
